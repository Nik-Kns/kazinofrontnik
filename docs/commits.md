...
## 72e6315 - refactor: изменен порядок шагов создания кампании - креатив перед сообщением
- **Новый порядок**: Основное → Цели → Сегменты → Каналы → **Креатив** → **Сообщение**
- **Шаг 5 (Креатив)**:
  - SMS: заглушка "не требуется"
  - Email/Push: большая drag & drop зона для загрузки файла
- **Шаг 6 (Сообщение)**:
  - SMS: textarea с лимитом 160 символов
  - Email/Push: табы "Визуальный редактор" / "HTML код"
  - Кнопки: ИИ генерация, Предпросмотр, A/B тест
- Упрощенный код, логичный порядок
